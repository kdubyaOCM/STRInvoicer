...
export const StepInvoice: React.FC<Props> = ({ data, config, onBack, onSaveDraft }) => {
  ...
  const totals = useMemo(() => {
    ...
    const allProcessedExpenses = [...data.autoReimbursables, ...data.reviewRows];
    ...
    return {
      ...
      feeBase,
      feeBaseLabel:
        config.feeBaseMode === 'gross_revenue' ? 'Gross OTA Revenue' : 'Net OTA Payouts',
      mgmtFeeAmount,
      totalReimbursables,
      totalDeductions,
      netToOwner,
  ...
  const handleDownloadCsv = () => {
    ...
    try {
      const rows = totals.reimbursableItems.map((item) => ({
        ...
        'Charged Amount':
          item.assigned_category === ExpenseCategory.SHARED
            ? item.debit_amount * ((item.split_percent || 100) / 100)
            : item.debit_amount,
        Note: item.note || '',
      }));
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex flex-col gap-4 mb-8 no-print">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            ...
            >
              {isPrinting ? (
                <div className="mr-2 h-4 w-4 border-2 border-white border-t-transparent animate-spin rounded-full" />
              ) : (
      ...
      <div
        ...
        className="bg-white shadow-2xl shadow-slate-200 mx-auto w-full max-w-[210mm] min-h-[297mm] text-slate-900 relative print:shadow-none print:w-full print:max-w-none print:m-0 flex flex-col"
      >
        <div className="h-3 w-full bg-slate-800 print:bg-black"></div>
        <div className="p-12 sm:p-16 flex-1 flex flex-col">
          <div className="flex justify-between items-start mb-16">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900 mb-1 uppercase">
                Owner Statement
              </h1>
            ...
            <div className="text-right">
              ...
              <p className="text-slate-500 whitespace-pre-line text-sm mt-1">
            ...
            <div className="grid grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="font-medium text-slate-900 text-sm">
                  {config.periodStart} <span className="text-slate-400 text-xs mx-1">to</span>{' '}
                  {config.periodEnd}
                </div>
              </div>
            </div>
          ...
          <table className="w-full mb-12">
            <thead>
              <tr className="border-b-2 border-slate-900">
                <th className="py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wide">
                  Description
                </th>
                <th className="py-3 text-right text-xs font-bold text-slate-500 uppercase tracking-wide w-40">
                  Amount
                </th>
              </tr>
            </thead>
              ...
              <tr>
                <td className="py-4 pl-2 pr-4">
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">
                    {config.mgmtFeePercent}% of {totals.feeBaseLabel} ($
                    {totals.feeBase.toLocaleString(undefined, { minimumFractionDigits: 2 })})
                  </div>
                </td>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50">
                    <td className="py-2 pl-4 pr-4">
                      ...
                      <div className="text-xs text-slate-500 flex items-center gap-2">
                        ...
                        {descSuffix && (
                          <span className="text-blue-600 font-medium">{descSuffix}</span>
                        )}
                      </div>
                    </td>
                    <td className="py-2 text-right align-top text-slate-600 text-sm">
              ...
              <div className="w-2/3 md:w-1/2 bg-slate-900 text-white rounded-lg p-6 shadow-lg">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-3xl font-bold">
                    $
                    {Math.abs(totals.netToOwner).toLocaleString(undefined, {
