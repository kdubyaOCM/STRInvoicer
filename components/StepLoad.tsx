...
import { Upload, FileText, Settings, AlertCircle, ArrowRight, CheckCircle, FileJson } from 'lucide-react';
    ...
    <label className={`relative group border-2 border-dashed rounded-xl p-6 transition-all duration-200 cursor-pointer block ${
      ...
      <div className="absolute top-4 right-4">
        {file ? (
          ...
          <span className="text-xs font-medium text-red-400 bg-red-50 px-2 py-1 rounded-full">Required</span>
        ) : (
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="text-sm text-slate-600">
            ...
            <p className="text-xs text-slate-400 mt-1">Load it here to restore your configuration immediately.</p>
          </div>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <FileCard title="OTA Export" desc="Booking statements" file={otaFile} onChange={handleFileChange(setOtaFile)} icon={FileText} required />
          <FileCard title="General Ledger" desc="Accounting export" file={glFile} onChange={handleFileChange(setGlFile)} icon={FileText} required />
          <FileCard title="Classification Map" desc="Mapping rules" file={mapFile} onChange={handleFileChange(setMapFile)} icon={Settings} />
        </div>
      </section>

      <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2"><Settings size={18} className="text-indigo-600" /><h2 className="text-sm font-bold uppercase tracking-wide text-slate-700">2. Configuration</h2></div>
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="text-sm font-medium text-slate-900 border-b border-slate-100 pb-2 mb-4">Reporting Settings</h3>
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1.5">Reporting Period</label>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <span className="text-xs text-slate-400 mb-1 block">Start Date</span>
                    <input type="date" className="block w-full rounded-lg border-slate-200 bg-slate-50 focus:bg-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow shadow-sm" value={config.periodStart} onChange={e => setConfig({...config, periodStart: e.target.value})} />
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 mb-1 block">End Date</span>
                    <input type="date" className="block w-full rounded-lg border-slate-200 bg-slate-50 focus:bg-white text-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow shadow-sm" value={config.periodEnd} onChange={e => setConfig({...config, periodEnd: e.target.value})} />
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1.5">Fee Base</label>
                  <select className="block w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" value={config.feeBaseMode} onChange={e => setConfig({...config, feeBaseMode: e.target.value as any})}>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium text-slate-900 border-b border-slate-100 pb-2 mb-4">Entity Details</h3>
              <div>
                 <label className="block text-xs font-medium text-slate-700 mb-1.5">Property Owner Name</label>
                 <input type="text" className="block w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" value={config.ownerName} onChange={e => setConfig({...config, ownerName: e.target.value})} placeholder="e.g. John Smith" />
              </div>
              <div>
                 <label className="block text-xs font-medium text-slate-700 mb-1.5">Property Manager Name</label>
                 <input type="text" className="block w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" value={config.managerName} onChange={e => setConfig({...config, managerName: e.target.value})} placeholder="e.g. Coastal Stays LLC" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="block text-xs font-medium text-slate-700 mb-1.5">Manager Contact</label>
                   <input type="text" className="block w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" value={config.managerContact} onChange={e => setConfig({...config, managerContact: e.target.value})} placeholder="Email or Phone" />
                </div>
                <div>
                   <label className="block text-xs font-medium text-slate-700 mb-1.5">Bank Details</label>
                   <input type="text" className="block w-full rounded-lg border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" value={config.managerBank} onChange={e => setConfig({...config, managerBank: e.target.value})} placeholder="Bank Account info" />
                </div>
              </div>
            </div>
          </div>
