 ...
 */
const parseNumber = (val: string | number | boolean | null | undefined): number => {
  if (typeof val === 'number') return val;
  if (!val || typeof val === 'boolean') return 0;
...
export const generateInitialMappings = (
  otaData: RawSpreadsheetRow[],
  glData: RawSpreadsheetRow[]
): MappingState => {
...
export const processData = (
  ...
  const isSingleColGl =
    mappings.gl.debit_amount === mappings.gl.credit_amount && !!mappings.gl.debit_amount;
  ...
  const allGlRows: CanonicalGlRow[] = files.glRaw
    .map((row) => {
      ...
      const mapEntry = Object.entries(files.classificationMap).find(
        ([k, v]) => k.toLowerCase() === account.toLowerCase()
      );
      if (mapEntry) {
